...
def create_batch_prompt_user_support(users, posts, total_points=24000):
    """
    Creates a prompt for the ORA API to distribute points among a list of posts.
    Expects:
      - users: list of creator names.
...
@app.post("/judge_user_support", summary="Distribute points among posts using ORA API")
def judge_posts_user_support(request: JudgeRequest):
    # print(f"Total points: {request.total_points}")
    # print("Users:", request.users)
    # print("Posts:", request.posts[0].keys())
    # Create the prompt using both users and posts
    prompt = create_batch_prompt_user_support(request.users, request.posts, request.total_points)
    # ORA API expects a messages array; here we send the prompt as a user message.
    messages = [{"role": "user", "content": prompt}]
